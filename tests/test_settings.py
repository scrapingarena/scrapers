...
def test_generic_oxylabs_credentials_are_supported(
    ...
    assert proxy is not None
    assert proxy.provider_name == "oxylabs"
    assert proxy.username == "premium-user"
...
@pytest.mark.parametrize(
    "username", ["customer-test", "customer-test-cc-DE-sessid-explicit"]
)
def test_proxy_preserves_explicit_credentials(
    monkeypatch: pytest.MonkeyPatch, username: str
) -> None:
    monkeypatch.setenv("OXYLABS_PROXIES_USERNAME", username)
    monkeypatch.setenv("OXYLABS_PROXIES_PASSWORD", "secret")
    proxy = configured_proxy("oxylabs")
    assert proxy is not None
