...
async def test_lightpanda_uses_server_proxy_and_default_context() -> None:
    class Page:
    ...
    class Browser:
        ...
        async def new_context(self, **_kwargs: Any) -> None:
            raise AssertionError("Lightpanda proxy must be configured on serve")
    ...
    scraper = LightpandaScraper(proxy=proxy)
